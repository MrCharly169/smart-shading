...
The GitHub release body is extracted exactly from the matching dated `CHANGELOG.md` section. It is never generated independently. Before a beta or stable tag can be published, the release workflow requires the fast suite, a clean real-Home-Assistant lifecycle, the real HA browser/Card suite, and an upgrade from the newest published Smart Shading tag. A protected isolated runner qualifies the published tag through HACS afterwards. See [docs/HA_E2E_LAB.md](docs/HA_E2E_LAB.md).
