...
The same zone can contain one or more native Home Assistant conditions. A binary presence zone can require somebody to be at a desk, for example, and numeric-state thresholds can be combined through AND, OR, or NOT groups. A false or unavailable condition disables only that glare zone. Each zone also has its own minimum sun elevation and may explicitly ignore the sector's Lux or external direct-sun confirmation for low-sun glare; facade azimuth, geometry, schedule and the zone's conditions still apply. The Advanced Card labels every individual cover target with its active mode, such as **Glare protection** or **Solar shading**.
