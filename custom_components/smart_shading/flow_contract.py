...
def sun_source_for_sector(
    ...
    Older entries predate the explicit marker. Geometry, Lux, and an external
    on/off confirmation are valid in either setup variant. A legacy external
    entity is authoritative over a simultaneously stored Lux source.
    """
    allowed = {"geometry", "lux", "external"}
    stored = str(sector.get("sun_source") or "")
    if stored in allowed:
        return stored
    if str(sector.get("sun_presence_entity") or "").strip():
        return "external"
