...
class _SmartShadingWizardMixin:
    ...
    def _feature_context_placeholders(
        ...
        return {
            **self._option_placeholders(),
            "feature_index": str(index + 1 if initial else 1),
            "feature_count": str(len(queue) if initial else 1),
    ...
    def _protected_zone_preview(
        ...
        return {
            **self._option_placeholders(),
            ...
            "calculation_marker": "⚠" if validation.status is not ProtectedZoneStatus.VALID else "✓" if evaluation.status is ProtectedZoneStatus.HIT else "○",
            "window_width": f"{float(zone_values['window_width_m']):.2f}",
            "window_height": f"{float(zone_values['window_height_m']):.2f}",
            "window_sill": f"{float(zone_values['window_sill_height_m']):.2f}",
            "object_distance": f"{float(zone_values['object_distance_m']):.2f}",
            "object_width": f"{float(zone_values['object_width_m']):.2f}",
            "object_height": f"{float(zone_values['object_height_m']):.2f}",
            "condition_count": str(condition_count),
            "sun_azimuth": f"{azimuth:.0f}" if azimuth is not None else "—",
            "sun_elevation": f"{elevation:.0f}" if elevation is not None else "—",
            "sun_confirmation": "✓" if confirmed else "○",
            "target_position": f"{evaluation.target.position:.0f} %" if evaluation.target is not None and evaluation.target.position is not None else "—",
            "target_tilt": f"{evaluation.target.tilt:.0f} %" if evaluation.target is not None and evaluation.target.tilt is not None else "—",
        ...
        return (
            {
                ...
                "geometry_source_count": str(source_counts["geometry"]),
                "lux_source_count": str(source_counts["lux"]),
                "external_source_count": str(source_counts["external"]),
                "temperature_room_count": str(temperature_rooms),
                "night_room_count": str(sum(bool(room.get("night_enabled")) for room in self.rooms)),
                "safety_source_count": str(sum(len(room.get("safety_blockers", [])) for room in self.rooms)),
                "review_issue_count": str(len(issues)),
                "review_issue_objects": ", ".join(dict.fromkeys(issue.split(":", 1)[0] for issue in issues if ":" in issue and not issue.startswith(("Covers assigned", "Behänge mehrfach")))) or "—",
                "sun_sources": sources,
