...
One house-level Operation and protection policy groups this behavior for dashboards, automations and voice assistants. By season and schedule follows the global calendar. Protection only forces thermally driven daytime modes off and immediately releases a latched Heat cycle. Year-round ignores seasonal months and weekdays while retaining the configured daily time window. Separate house permissions gate configured Night and calculated Glare Protection year-round. Safety, source-quality holds and hard device limits remain independent and Safety is never switchable through this policy. Rooms inherit the house mode and schedule by default and retain explicit Advanced overrides. The native house and room Selects and `smart_shading.set_operating_profile` service use persisted runtime overrides.
...
Each protected zone may also contain native Home Assistant activation conditions. Multiple top-level conditions are ANDed, while compound condition groups can express AND, OR, or NOT. Typical examples are a binary presence zone that must be on, a weather entity that must be `sunny`, or a numeric sensor that must be above or below a threshold. Empty conditions mean that sun and geometry alone control the zone. Per-zone activation and release delays debounce the aggregate condition result; new zones default to 60 seconds continuously true before activation and a 300-second release grace after false or unavailable input. The latch and its deadline survive restart. Hysteresis applies only to conditions: leaving the facade, minimum elevation or calculated footprint still ends the glare hit immediately. A false or unavailable condition disables only that zone after its configured grace; it does not block ordinary Solar, Comfort, Heat Protection, or another protected zone. Referenced condition entities and exact deadlines trigger an event-driven reevaluation. A separate per-zone switch may disable the sector's Lux or external direct-sun confirmation for low-sun glare. The zone's own minimum sun elevation, facade azimuth, geometry and conditions still gate it; ordinary Solar and Comfort continue to use the sector source unchanged.

At any time of year, a valid protected-zone intersection may request glare protection even when the general shading schedule is inactive or the room is too cool for temperature-based shading. It starts from the active Solar or Comfort target, otherwise from Open, and may only make that target more protective. Multiple zones select the most protective axis values. Safety, manual control or pause, unavailable required inputs, Night, and Heat Protection remain higher priority.
